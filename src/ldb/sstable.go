...
func NewSSTableV1(filepath string) (t *SSTableV1, err error) {
    ...
    // parse domain
    t.domain, err = util.NewEncodedMappedData(mmap_data[pos:])
    if err != nil {
        return
    }
    ...
    // parse domain
    t.table, err = util.NewEncodedMappedData(mmap_data[pos:])
    if err != nil {
        return
    }
    pos += len(t.table.Buf())
    ...
    // parse start key
    t.start_key, err = util.NewEncodedMappedData(mmap_data[pos:])
    if err != nil {
        return
    }
    ...
    // parse end key
    t.end_key, err = util.NewEncodedMappedData(mmap_data[pos:])
    if err != nil {
        return
    }
